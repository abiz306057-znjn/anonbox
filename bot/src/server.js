 ...
 * نسخه ۳.۰ — اصلاح‌شده
 *
 * ⚠️ تفاوت با نسخه قبلی:
 *   - همه routes با یه mount کار می‌کنن: app.use('/api', routes.xxx)
 *   - auth در خود routeها مدیریت می‌شه (middlewares.verifyTelegram)
 *   - ترتیب mount مهمه (public قبل از protected)
 */
...
if (config.env.isDev) {
  app.use((req, res, next) => {
    ...
    res.on('finish', () => {
      ...
      // فقط API رو لاگ کن (نه static)
      if (req.originalUrl.startsWith('/api') || req.originalUrl === '/health') {
        console.log(`${emoji} ${method} ${status} ${req.originalUrl} (${duration}ms)`);
      }
    });
...
// ============================================================
//  API Routes
//  ⚠️ همه با یه mount — auth تو خود routeها
// ============================================================

// ---- کاربران ----
// شامل: /api/me, /api/profile, /api/slug, /api/users
app.use('/api', routes.user);

// ---- پیام‌ها ----
// شامل: /api/messages
app.use('/api', routes.message);

// ---- پرداخت + بسته‌ها ----
// شامل: /api/payments, /api/packages
app.use('/api', routes.payment);

// ---- دعوت ----
// شامل: /api/referrals
app.use('/api', routes.referral);

// ---- ادمین + آمار ----
// شامل: /api/admin, /api/stats
